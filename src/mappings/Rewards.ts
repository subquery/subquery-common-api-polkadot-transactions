...
async function buildRewardEvents<A>(
    ...
    const [, savingPromises] = block.events.reduce<[A, Promise<void>[]]>(
        (accumulator, eventRecord, eventIndex) => {
            ...
            let [account, amount] = decodeDataFromReward(eventRecordToSubstrateEvent(eventRecord))
...
export async function handleGenericForTxHistory(event: SubstrateEvent, address: string, fieldCallback: (element: HistoryElement) => Promise<HistoryElement>): Promise<void> {
